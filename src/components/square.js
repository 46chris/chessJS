...
function setBoard(row, col) {
  ...
  // Black pawns
  if (row === 1) {
    return <Pawn pos={[row, col]} name="PAWN" color={color} />;
  }
  // White pawns
  if (row === 6) {
    return <Pawn pos={[row, col]} name="PAWN" color={color} />;
...
export default function Square({ black, pos }) {
  ...
  const movePiece = (item) => {
    if (item.name === 'PAWN') {
      ...
      setPiece(
        <Pawn name={item.name} pos={[pos[0], pos[1]]} color={item.color} setSquare={setSquare} />
      );
    }
  };
  ...
  return (
    <div
      ...
      ref={drop}
    >
      {piece == null ? null : <piece.type {...piece.props} setSquare={setSquare} />}
    </div>
  );
}
